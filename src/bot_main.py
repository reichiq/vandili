...
def latex_to_png(latex: str) -> str:
    ...
    fig.savefig(tmp.name, bbox_inches="tight", pad_inches=0.3, transparent=True)
    plt.close(fig)
    return tmp.name
...
EXCHANGE_PATTERN = re.compile(
    r"(?ix)"                           # флаги: i — без учёта регистра, x — игнорировать пробелы в паттерне
    r"(?:"
    r"    (\d+(?:[.,]\d+)?)\s+"        # 1) необязательное число (amount: 1, 1.5, 100)
    r")?"
    r"([а-яёa-z$€₽¥]+)"                # 2) валюта-от (доллар, USD, $, рубль и т.п.)
    r"(?:\s+(?:в|to))?\s+"             # опциональное «в» или «to»
    r"([а-яёa-z$€₽¥]+)"                # 3) валюта-к (сум, UZS и т.п.)
)
...
# ──────────────────────────────────────────────────────────────────────
#  >>>  handle_msg – версия с разделением формулы и обычного текста  <<<
# ──────────────────────────────────────────────────────────────────────
async def handle_msg(
    ...
    # A. Если есть формула, обрабатываем как формулу
    user_data = user_images_text.pop(uid, None)
    if user_data:
        formula = user_data.get("formula")
        ...
        if formula:
            ...
            if not user_input:
                await message.answer(
                    "✍️ Сформулируй вопрос к этой формуле, и я отвечу!",
                    **thread_kwargs(message)
                )
                return

            prompt = (
                "Ты — опытный преподаватель математики. Объясняй всё максимально подробно и при этом "
                "простым, понятным языком. Избегай громоздких формулировок, разжёвывай каждый шаг "
                "и давай маленькие примеры там, где это уместно.\n\n"
            ...
            await message.answer(
                "⚠️ Картинка считана, но текст не найден. Сформулируй свой вопрос вручную!",
                **thread_kwargs(message)
            )
            return

    # B. Всё остальное: обычные сообщения
    ...
    if image_url:
        ...
        for chunk in split_smart(gemini_text, TELEGRAM_MSG_LIMIT):
            await message.answer(chunk, parse_mode="HTML", **thread_kwargs(message))
    else:
...
async def generate_and_send_gemini_response(cid, full_prompt, show_image, rus_word, leftover):
    ...
    try:
        ...
        unknown_triggers = ["извин", "не знаю", "не могу дать", "не могу ответить", "нет информации", "у меня нет сведений", "у меня нет данных", "это дата находится в будущем", "ещё не произошло", "дата еще не наступила", "до моего последнего обновления"]

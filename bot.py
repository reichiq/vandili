...
# Любые триггеры для показа изображения
IMAGE_TRIGGERS = [
...
# Список нежелательных фраз (чтобы не «палить», что бот не умеет напрямую показывать картинки)
...
def format_gemini_response(text: str) -> str:
    ...
    # Удаляем «заглушки» вида [вставьте фото...]
    ...
    # Удаляем нежелательные отговорки
...
def get_safe_prompt(text: str) -> str:
    """Извлекаем осмысленный prompt (ключевые слова) для поиска картинки."""
    text = re.sub(r'[.,!?\-\n]', ' ', text.lower())
...
async def get_unsplash_image_url(prompt: str, access_key: str) -> str:
    """Берём случайное изображение c Unsplash по запросу prompt."""
...
def split_text(text: str, max_length: int = 950):
    """Разделяем длинный текст по точкам, чтобы не рвать предложения."""
    parts = []
...
async def handle_message(message: Message):
    ...
    # Если пользователь спрашивает о владельце
    ...
    # Храним контекст диалога (сжимаем до 5 сообщений)
    ...
    try:
        ...
        # Формируем prompt для поиска картинки (если нужно)
        ...
        # Если попросили фото и url есть
        if image_url and triggered:
            logging.info("[BOT] Пробую скачать файл и отправить фото...")
            try:
                async with aiohttp.ClientSession() as session:

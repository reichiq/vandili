...
async def handle_photo_message(message: Message):
    ...
    try:
        ...
        if text_raw and not extracted_latex:
            # Если формула не распознана, используем распознанный текст
            user_images_text[message.from_user.id] = text_raw
        # Дальнейшая обработка производится в handle_all_messages_impl
        await handle_all_messages_impl(message, message.caption or "")
    except Exception as e:
...
async def handle_all_messages(message: Message):
    ...
    if formula:
        prompt = (
            ...
            f"\\[{formula}\\]\n\n"
            f"Покажи решение пошагово. Объясни каждый шаг, если он неочевиден. Не добавляй преобразований, если они не нужны. Не пиши модуль, если это не вытекает из условия."
        )
...
async def handle_all_messages_impl(message: Message, user_input: str):
    ...
    if uid in user_images_text:
        ...
        is_latex_formula = bool(re.search(r'[=+\-\^\\]', extracted)) and extracted.startswith("\\")
        if is_latex_formula:
            ...
            try:
                ...
                logging.warning(f"[BOT] Ошибка отрисовки исходной формулы: {e}")
                await message.answer(response or "⚠️ Не удалось отрисовать формулу, но вот ответ:")
        else:
            prompt = (
...
def generate_short_caption(rus_word: str) -> str:
    short_prompt = (
        "ИНСТРУКЦИЯ: Ты — творческий помощник, который умеет писать очень короткие, дружелюбные подписи на русском языке. "
        "Не упоминай, что ты ИИ или Google. Старайся не превышать 15 слов.\n\n"
        f"ЗАДАЧА: Придумай одну короткую, дружелюбную подпись для картинки с «{rus_word}». Можно с лёгкой эмоцией или юмором, не более 15 слов."
    )
    try:

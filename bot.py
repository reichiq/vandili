...
def normalize_currency_name(name: str) -> str:
    ...
    # Если синонима нет, берём верхний регистр (например, "USD", "EUR" и т.д.)
...
async def handle_all_messages(message: Message):
    ...
    # Расширенное регулярное выражение для любых валют
    ...
    # Обработка запроса погоды (например, "погода в ташкенте" / "погода в Tashkent")
    if "погода" in lower_input:
        # Расширенный шаблон для названий городов (рус/латин, возможен дефис), + IGNORECASE
...
async def handle_msg(message: Message, recognized_text: str = None):
    ...
    # Курс валют (любой)
    exchange_match = re.search(r"(\d+(?:[.,]\d+)?)\s*([a-zа-яё$€₽¥]+)\s*(в|to)\s*([a-zа-яё$€₽¥]+)", lower_inp)
    if exchange_match:
        amount_str = exchange_match.group(1).replace(',', '.')
        try:
            amount = float(amount_str)
        ...
        exchange_text = await get_exchange_rate(amount, from_curr, to_curr)
        if exchange_text:
            await bot.send_message(chat_id=cid, text=exchange_text, **thread_kwargs(message))
        else:
            await bot.send_message(chat_id=cid, text="Не удалось получить курс валюты.", **thread_kwargs(message))
        return

    # Погода (любой город, количество дней)
    if "погода" in lower_inp:
        weather_match = re.search(r"погода(?:\s+в)?\s+([a-zа-яё\-\s]+)(?:\s+на\s+(\d+|неделю))?", lower_inp, re.IGNORECASE)
        if weather_match:
            city = weather_match.group(1).strip()
            days_str = weather_match.group(2)
            days = 7 if days_str == "неделю" else int(days_str) if days_str else 1
            weather_info = await get_weather_info(city, days)
            if weather_info:
                await bot.send_message(chat_id=cid, text=weather_info, **thread_kwargs(message))
            else:
                await bot.send_message(chat_id=cid, text="Не удалось получить данные о погоде.", **thread_kwargs(message))
            return

    # Обработка "покажи" / изображения

...
async def get_floatrates_rate(from_curr: str, to_curr: str) -> float:
    """
    Получает курс из floatrates.com для пары (from_curr -> to_curr).
    from_curr и to_curr должны быть ISO-кодами (например, 'usd', 'rub').
    Если данные не получены, возвращает None.
    """
...
async def get_exchange_rate(amount: float, from_curr: str, to_curr: str) -> str:
    """
    Получает курс для конвертации между любыми валютами, поддерживаемыми floatrates.
    Рассчитывает результат через умножение на курс.
    """
    ...
    # В ответе не указываем источник, выводим только: "Курс 100 USD – ... на [дата] 😊"
...
async def geocode_city(city_name: str) -> dict:
    ...
    if data:
        return data
    # Если не найдено, попробуем перевести через Google Translate
    try:
...
async def handle_all_messages(message: Message):
    ...
    # Обработка запроса курса валют (например, "100 долларов в рублях", "100 баксов в KRW")
    ...
    # Обработка запроса погоды (например, "погода в ташкенте", "погода в москве на 3 дня")
    ...
    # Если не курс и не погода – передаём запрос в обработчик Gemini и/или для картинок

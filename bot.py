...
def call_deepseek_chat_api(chat_messages: list[dict], api_key: str) -> str:
    """
    Отправляем запрос к DeepSeek Chat API.
    Ожидается, что chat_messages имеет формат:
      [
         {"role": "system", "content": "..."}, 
         {"role": "user", "content": "..."}, 
         ...
    ...
    """
    url = "https://platform.deepseek.ai/api/chat"  # Используем URL согласно документации
    headers = {
        "Authorization": f"Bearer {api_key}",
        "Content-Type": "application/json",
    }
    data = {
        "model": "deepseek/deepseek-r1:free",  # Используйте название модели согласно документации
...
def deepseek_generate_content(messages: list[dict]) -> str:
    """
    Преобразуем внутреннюю структуру messages в формат Chat API и вызываем DeepSeek.
    """
    chat_messages = []
    for msg in messages:
        role = msg["role"]
...
def format_deepseek_response(text: str) -> str:
    """
    Форматируем ответ DeepSeek к HTML, заменяем "* " на "• " и добавляем переносы строк между пунктами.
    """
    text = escape(text)
...
async def generate_and_send_deepseek_response(cid, full_prompt, show_image, rus_word, leftover, thread_id):
    """
    Генерация ответа через DeepSeek. Обнуляем историю, добавляем system и user сообщения, затем вызываем deepseek_generate_content.
    """
    chat_history[cid] = []
    chat_history[cid].append({"role": "system", "parts": [SYSTEM_PROMPT]})
    if show_image and rus_word and not leftover:
...
async def generate_and_send_deepseek_response(cid, full_prompt, show_image, rus_word, leftover, thread_id):
    """
    Генерация ответа через DeepSeek. Обнуляем историю, добавляем system и user сообщения, вызываем deepseek_generate_content.
    """
    chat_history[cid] = []
    chat_history[cid].append({"role": "system", "parts": [SYSTEM_PROMPT]})
    if show_image and rus_word and not leftover:
